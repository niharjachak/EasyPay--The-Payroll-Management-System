...
public class PayrollPolicy {
    ...
    public PayrollPolicy(@Positive(message = "Policy ID must be a positive number.") int policyId,
			@NotNull(message = "Policy name must not be null.") int policyName,
			@NotBlank(message = "Policy description must not be blank.") String policyDesc,
			@NotNull(message = "Base salary must not be null.") @DecimalMin(value = "0.0", inclusive = true, message = "Base salary must be 0 or greater.") double baseSalary,
			@NotNull(message = "Overtime rate must not be null.") @DecimalMin(value = "0.0", inclusive = true, message = "Overtime rate must be 0 or greater.") double overtimeRate,
			@NotNull(message = "Tax percentage must not be null.") @DecimalMin(value = "0.0", inclusive = true, message = "Tax percentage must be 0 or greater.") @DecimalMax(value = "100.0", inclusive = true, message = "Tax percentage cannot exceed 100.") double taxPercentage) {
		super();
